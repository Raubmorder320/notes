...
$\alpha=(\alpha_1,...,\alpha_n)$ - мультииндекс(вектор-строка)
...
пусть $f\in C^1(E)$
$\forall \ 1 \le k \le r \ \ \ \forall x_{i_1},...,x_{i_k} \ \ \forall x \in E$
$\exists f^{(k)}_{x_{j_1},...,x_{j_{k}}}(x)\in C(E)$
<font color="#ffff00">тогда</font> $f \in C^{r}(E)$
#### Теорема
$r \ge 2 \ \ E\subset R^n \ , \ \ n \ge 2,E$-открыто
$f\in C^r(E)$
$i_1,.....i_r$     $j_1,.....j_r$ - отличаются только перестановкой
...
$j_{k}\ne j_{k+1}$
Рассмотрим:
$f^{(r+1)}_{x_{j_1},...,x_{j_{k}},x_{j_{k+1}},..,x_{j_{r+1}}}(x)$
$f^{(r+1)}_{x_{j_1},...,x_{j_{k+1}},x_{j_{k}},..,x_{j_{r+1}}}(x)$
...
Применяем следствие ТСП:
(2),(3)=>$g''_{x_{j_{k+1}},x_{j_{k}}}=g''_{x_{j_{k}},x_{j_{k+1}}}$ ==(4)==
...
<font color="#ffff00">(тут в целом чтд, т.к. перестановку можно породить перемещением двух любых)</font>
По-Широковски:
